...
function processSecondaryStructure(cif: Cif, structure: Structure, asymIdDict: { [k: string]: string }) {
  ...
  if (sc) {
    ...
    for (i = 0, il = sc.beg_auth_seq_id.length; i < il; ++i) {
      var helixType = sc.pdbx_PDB_helix_class ? parseInt(sc.pdbx_PDB_helix_class[i]) : NaN
      if (!Number.isNaN(helixType)) {
        begIcode = sc.pdbx_beg_PDB_ins_code ? sc.pdbx_beg_PDB_ins_code[i] : "?"
        endIcode = sc.pdbx_end_PDB_ins_code ? sc.pdbx_end_PDB_ins_code[i] : "?"
        helices.push([
...
function processConnections(cif: Cif, structure: Structure, asymIdDict: { [k: string]: string }) {
  ...
  if (sc) {
    ...
    for (var i = 0, il = sc.id.length; i < il; ++i) {
      ...
      var inscode1 = sc.pdbx_ptnr1_PDB_ins_code ? sc.pdbx_ptnr1_PDB_ins_code[i] : "?"
      var altloc1 = sc.pdbx_ptnr1_label_alt_id ? sc.pdbx_ptnr1_label_alt_id[i] : "?"
      var filt1 = (
      ...
      var atomIndices1 = atomIndicesCache[filt1]
      if (!atomIndices1) {
        var filtering1 = new Filter(filt1)
        if (filtering1.filter.error) {
          if (Debug) Log.warn('invalid filter for connection', filt1)
          continue
        }
        atomIndices1 = structure.getAtomIndices(filtering1)
        atomIndicesCache[filt1] = atomIndices1
      }

      var inscode2 = sc.pdbx_ptnr2_PDB_ins_code ? sc.pdbx_ptnr2_PDB_ins_code[i] : "?"
      var altloc2 = sc.pdbx_ptnr2_label_alt_id ? sc.pdbx_ptnr2_label_alt_id[i] : "?"
      var filt2 = (
      ...
      var atomIndices2 = atomIndicesCache[filt2]
      if (!atomIndices2) {
        var filtering2 = new Filter(filt2)
        if (filtering2.filter.error) {
          if (Debug) Log.warn('invalid filter for connection', filt2)
          continue
        }
        atomIndices2 = structure.getAtomIndices(filtering2)
        atomIndicesCache[filt2] = atomIndices2
      }

      // cases with more than one atom per filtering
      // - #altloc1 to #altloc2
      // - #model to #model
      ...
      if (k === 0 || l === 0) {
        if (Debug) Log.warn('no atoms found for', filt1, filt2)
        continue
      }
      ...
      for (var j = 0; j < l; ++j) {
        ...
        if (ap1 && ap2) {
          structure.bondStore.addBond(
            ap1, ap2, getBondOrder(sc.pdbx_value_order ? sc.pdbx_value_order[i] : '?')
          )
        } else {
...
function processEntities(cif: Cif, structure: Structure, chainIndexDict: { [k: string]: Set<number> }) {
  if (cif.entity) {
    ...
    var n = e.id.length
    for (var i = 0; i < n; ++i) {
      var description = e.pdbx_description ? e.pdbx_description[i] : undefined
      var type = e.type ? e.type[i] : undefined
