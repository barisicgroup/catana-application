...
export interface Contacts {
  ...
  featureSet: BitArray_Legacy
}

export interface FrozenContacts extends Contacts {
  contactSet: BitArray_Legacy
...
export function createContacts (features: Features): Contacts {
  ...
  const featureSet = new BitArray_Legacy(types.length, false)
...
export function createFrozenContacts (contacts: Contacts): FrozenContacts {
  ...
  const contactSet = new BitArray_Legacy(contacts.contactStore.count, true)
...
export function getContactData (contacts: FrozenContacts, structure: Structure, params: ContactDataParams): ContactData {
  ...
  let filterSet: BitArray_Legacy | BitArray_Legacy[] | undefined
  if (p.filterSele) {
    if (Array.isArray(p.filterSele)) {
      filterSet = p.filterSele.map(filt => {
        return structure.getAtomSet(new Filter(filt))
      })
    } else {
      filterSet = structure.getAtomSet(new Filter(p.filterSele))
    }
  }
