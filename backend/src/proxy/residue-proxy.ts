...
import BondProxy from './bond-proxy'
...
class ResidueProxy {
  ...
  eachAtom(callback: (ap: AtomProxy) => void, filter?: Filter) {
    ...
    if (filter && filter.atomOnlyTest) {
      const atomOnlyTest = filter.atomOnlyTest
  ...
  // Catana extension
  // Based on a NGL filter language query
  isLigand() {
    return (!this.isPolymer() || this.isHetero()) && !(this.isWater() || this.isIon());
  }
  ...
  getResname1() {
    const resName = this.resname.toUpperCase();
    return AA1[resName] ?? NA1[resName] ?? 'X';
  }
  ...
  public getBackboneCentroid(): Vector3 {
    return this.getAtomCentroid((ap: AtomProxy) => ap.isBackbone());
  }

  public getSidechainCentroid(): Vector3 {
    return this.getAtomCentroid((ap: AtomProxy) => ap.isSidechain());
  }

  /**
   * Computes the centroid of the atoms for whose predicate returns true.
   */
  public getAtomCentroid(predicate?: (a: AtomProxy) => boolean): Vector3 {
    let result = new Vector3(0, 0, 0);
    let tmpVec = new Vector3();
    let atProxy = new AtomProxy(this.structure);
    let successCount = 0;

    for (let i = this.atomOffset; i <= this.atomEnd; ++i) {
      atProxy.index = i;

      if (predicate === undefined || predicate(atProxy)) {
        result.add(atProxy.positionToVector3(tmpVec));
        ++successCount;
      }
    }

    return result.divideScalar(successCount);
  }

  public removeAtomWhere(predicate: (a: AtomProxy) => boolean): void {
    this.structure.removeAtomWhere(predicate, this.atomOffset, this.atomCount);
  }

  public removeAtomsWhere(predicate: (a: AtomProxy) => boolean): void {
    this.structure.removeAtomsWhere(predicate, this.atomOffset, this.atomCount);
  }

  // Removes OH atom pair from COOH carboxyl group
  public removeOHAtomsfromCOOH(): void {
    let indicesToRemove: number[] = [];

    this.eachAtom(function (ap: AtomProxy) {
      if (indicesToRemove.length === 0 && ap.element === 'O') {
        let bondedCarbonIdx = -1;
        let bondedHydrogenIdx = -1;

        ap.eachBondedAtom(function (bondedAp: AtomProxy) {
          if (bondedAp.atomname === 'C') {
            bondedCarbonIdx = bondedAp.index;
          } else if (bondedAp.element === 'H') {
            bondedHydrogenIdx = bondedAp.index;
          }
        });

        if (bondedCarbonIdx >= 0) {
          let removeH = false;
          ap.eachBond(function (bp: BondProxy) {
            if ((bp.atomIndex1 === bondedCarbonIdx || bp.atomIndex2 === bondedCarbonIdx) && bp.bondOrder === 1) {
              indicesToRemove.push(ap.index);
              removeH = true;
            }
          });

          if (bondedHydrogenIdx > 0 && removeH) {
            indicesToRemove.push(bondedHydrogenIdx);
          }
        }
      }
    });
    ...
    for (let i = indicesToRemove.length - 1; i >= 0; --i) {
      this.structure.removeAtomsStartingAtIndex(indicesToRemove[i], 1);
    }
  }

  // Removes hydrogen atom from amine group
  public removeHAtomFromNH2(): void {
    this.removeAtomWhere(function (ap: AtomProxy) {
      let hasBondToN = false;
      if (ap.element === 'H') {
        ap.eachBondedAtom(function (bondedAp: AtomProxy) {
          if (bondedAp.atomname === 'N') {
            hasBondToN = true;
          }
        });
      }
