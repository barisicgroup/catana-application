...
class ChainProxy {
  ...
  eachResidue (callback: (rp: ResidueProxy) => void, filter?: Filter) {
    ...
    if (filter && filter.test) {
      const residueOnlyTest = filter.residueOnlyTest
      if (residueOnlyTest) {
  ...
  eachPolymer (callback: (p: Polymer) => void, filter?: Filter) {
    let rStartIndex = 0
    let rNextIndex = 0
    const test = filter ? filter.residueOnlyTest : undefined
    ...
    for (let i = offset + 1; i < end; ++i) {
      ...
      if (bbType1 !== UnknownBackboneType && bbType1 === bbType2) {
        ...
        if (bbType1 !== UnknownBackboneType) {
          if (rp1.index - rStartIndex >= 1) { // Equality is Catana extension to allow also polymers with two residues. Reason: polymer-based rendering (e.g. cartoon)
    ...
    if (rNextIndex - rStartIndex >= 1) { // Equality is Catana extension to allow also polymers with two residues. Reason: polymer-based rendering (e.g. cartoon)
