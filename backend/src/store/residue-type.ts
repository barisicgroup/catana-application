...
import {
  ...
  CgProteinBackboneType, CgRnaBackboneType, CgDnaBackboneType, PurineNucleobaseType,
  ChemCompProtein, ChemCompRna, ChemCompDna, ChemCompSaccharide,
  AA3, NucleobaseTypeAtoms, RnaBases, DnaBases, Bases, IonNames, WaterNames, SaccharideNames,
  ProteinBackboneAtoms, NucleicBackboneAtoms, ResidueTypeAtoms, PyrimidineNucleobaseType
...
export default class ResidueType {
  ...
  constructor (readonly structure: Structure, resname: string, atomTypeIdList: number[], hetero: boolean, chemCompType: string, bonds?: ResidueBonds) {
    ...
    let rungEndIndex
    if (NucleobaseTypeAtoms[PurineNucleobaseType].baseNames.includes(resname)) {
  ...
  isPurineNucleotide() {
    return this.isNucleic() && this.hasAtomWithName(NucleobaseTypeAtoms[PurineNucleobaseType].baseNames);
  }

  isPyrimidineNucleotide() {
    return this.isNucleic() && !this.isPurineNucleotide() && this.hasAtomWithName(NucleobaseTypeAtoms[PyrimidineNucleobaseType].baseNames);
  }
