...
export interface ComponentSignals {
  ...
  lockedChanged: Signal // on locked change
}
...
abstract class Component implements IUpdateable {
  ...
  private _requestReprsUpdate: boolean = false;   // Catana extension
  private _requestReprsRebuild: boolean = false;  // Catana extension
  ...
  get backendOnly() { return this.parameters.backendOnly }
  get locked() { return this.parameters.locked; }

  /** @override */
  public onUpdate(delta: number) {
    if (this._requestReprsRebuild) {
      // If we perform a rebuild, we will "cancel" scheduled update
      // as it is not necessary anymore
      this.rebuildRepresentations();
      this._requestReprsRebuild = false;
      this._requestReprsUpdate = false;
    }
    else if (this._requestReprsUpdate) {
   ...
   * 
   * @remark Catana addition
   * 
   * @param object object to search for
   * @returns true if the object is part of this component in some way
   */
  hasObject(object: any): boolean {
    return this.object === object;
  }
  abstract supportsColorScheme(scheme: any): boolean;
  ...
  public requestRepresentationsUpdate() {
    this._requestReprsUpdate = true;
  }

  public requestRepresentationsRebuild() {
    this._requestReprsRebuild = true;
  }
  ...
  setStatus(value: string): Component {
  ...
  public setLocked(value: boolean): Component {
    if (this.parameters.locked !== value) {
      this.parameters.locked = value;
      this.signals.lockedChanged.dispatch(value);
    }
   ...
   * @remark Catana extension
   * 
   * @param pos local position to transform
   * @returns corresponding world position
   */
  public localToWorldPosition(pos: Vector3): Vector3 {
   ...
   * @remark Catana extension
   * 
   * @param dir local direction to transform
   * @returns corresponding world direction
   */
  public localToWorldDirection(dir: Vector3): Vector3 {
    const rotMatrix = new Matrix4().extractRotation(this.matrix);
    return dir.clone().applyMatrix4(rotMatrix);
  }
