...
export interface RepresentationElementSignals extends ElementSignals {
  ...
  filterChanged: Signal      // on filter change
}
...
class RepresentationElement extends Element {
  ...
  setFilter(string: string) {
    ...
    if (repr.setFilter) {
      let callEvent = repr.filterString !== undefined && repr.filterString.trim() !== string.trim();

      repr.setFilter(string);

      if (callEvent) {
        this.signals.filterChanged.dispatch(string);
      }
    }
    return this
  }
