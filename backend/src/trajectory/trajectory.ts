 ...
 * @property {String} filt - to restrict atoms used for superposition
...
export interface TrajectoryParameters {
  ...
  filt: string  // to restrict atoms used for superposition
...
class Trajectory {
  ...
  filter: Filter  // filter to restrict atoms used for superposition
  structure: Structure
  ...
  constructor (trajPath: string, structure: Structure, params: Partial<TrajectoryParameters> = {}) {
    ...
    this.filter = new Filter(
      defaults(params.filt, 'backbone and not hydrogen')
    )

    this.filter.signals.stringChanged.add(() => {
      this.filteringIndices = this.structure.getAtomIndices(this.filter)!
      this._resetCache()
  ...
  setStructure (structure: Structure) {
    ...
    this.backboneIndices = this._getIndices(
      new Filter('backbone and not hydrogen')
    )
    this._makeAtomIndices()
    this._saveStructureCoords()

    this.filteringIndices = this._getIndices(this.filter)
    this._resetCache()
    this._saveInitialCoords()
    this.setFrame(this._currentFrame)
  }
  ...
  _makeSuperposeCoords () {
    const n = this.filteringIndices.length * 3
    ...
    for (let i = 0; i < n; i += 3) {
      const j = this.filteringIndices[ i / 3 ] * 3
  ...
  setFrame (i: number, callback?: Function) {
    if (i === undefined || this._disposed) return this
  ...
  _process (i: number, box: ArrayLike<number>, coords: Float32Array, frameCount: number) {
    ...
    if (this.filteringIndices.length > 0 && this.coords1 && this.superpose) {
