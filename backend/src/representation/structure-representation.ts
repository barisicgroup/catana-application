...
import { Assembly, Volume } from '../catana';
...
abstract class StructureRepresentation extends Representation {
  ...
  constructor (structure: Structure, viewer: Viewer, params: Partial<StructureRepresentationParameters>) {
     ...
     * @type {StructureView}
     */
    this.structureView = this.structure.getView(this.filter)
  ...
  init (params: Partial<StructureRepresentationParameters>) {
    ...
    this.filter.signals.stringChanged.add((/* filt */) => {
      this.build()
    })
   ...
   * @alias StructureRepresentation#setFilter
   * @param {String} string - filter string, see {@tutorial selection-language}
   * @param {Boolean} [silent] - don't trigger a change event in the filter
   * @return {StructureRepresentation} this object
   */
  setFilter (string: string, silent?: boolean) {
    this.filter.setString(string, silent)
  ...
  getParameters () {
    const params = Object.assign(
      super.getParameters(),
      {
        filt: this.filter ? this.filter.string : undefined,
