...
import { Structure, Vector3, Volume } from '../catana';
...
class MolecularSurfaceRepresentation extends StructureRepresentation {
  ...
  prepareData (sview: StructureView, i: number, callback: (i: number) => void) {
    ...
    if (!info.molsurf || info.filt !== sview.filter.string) {
      if (this.filterSele) {
        const sviewFilter = sview.structure.getView(new Filter(this.filterSele))
        ...
        sview = sview.getView(
          new Filter(sview.getAtomSetWithinFilter(asWithin, 3).toFiltString())
        )
  ...
  prepare (callback: () => void) {
    if (this.__forceNewMolsurf || this.__filt !== this.filter.string ||
    ...
    const after = () => {
      this.__filt = this.filter.string
      this.__surfaceParams = JSON.stringify(this.getSurfaceParams())
