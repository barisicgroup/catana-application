 ...
 *         filt: "RET", showAxes: false, showBox: true, radius: 0.2
 *     } );
 *     o.addRepresentation( "ball+stick", { filt: "RET" } );
 *     o.addRepresentation( "axes", {
 *         filt: ":B and backbone", showAxes: false, showBox: true, radius: 0.2
 *     } );
 *     stage.autoView();
...
class AxesRepresentation extends StructureRepresentation {
  ...
  getPrincipalAxes (): PrincipalAxes {
    ...
    if (assembly) {
      filter = assembly.partList[ 0 ].getFilter()
    }

    return this.structureView.getPrincipalAxes(filter)
  }
