...
class DistanceRepresentation extends MeasurementRepresentation {
  ...
  getDistanceData (sview: StructureView, atomPair: AtomPair) {
    ...
    const filt1 = new Filter()
    const filt2 = new Filter()
    ...
    const filtered = sview.getAtomSet()
    ...
    atomPair.forEach((pair, i) => {
      ...
      if (typeof(v1) === 'number' && Number.isInteger(v1) && typeof(v2) === 'number' && Number.isInteger(v2)) {
        if (filtered.get(v1) && filtered.get(v2)) {
          ap1.index = v1
          ap2.index = v2
        } else {
          j += 1
        ...
        filt1.setString(v1 as string)
        filt2.setString(v2 as string)

        var atomIndices1 = sview.getAtomIndices(filt1)
        var atomIndices2 = sview.getAtomIndices(filt2)
    ...
    var bondSet = new BitArray_Legacy(bondStore.count, true)
