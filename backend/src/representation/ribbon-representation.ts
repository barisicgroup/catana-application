...
import AllAtomSpline, { AllAtomSplineParameters } from '../geometry/spline'
...
class RibbonRepresentation extends StructureRepresentation {
  ...
  getSplineParams (params?: Partial<AllAtomSplineParameters>) {
  ...
  createData (sview: StructureView) {
    ...
    this.structure.eachPolymer(polymer => {
      ...
      var spline = new AllAtomSpline(polymer, this.getSplineParams())
  ...
  updateData (what: {position?: boolean, radius?: boolean, scale?: boolean, color?: boolean}, data: {polymerList: Polymer[], bufferList: RibbonBuffer[]}) {
    ...
    for (i = 0; i < n; ++i) {
      var bufferData = {}
      var spline = new AllAtomSpline(data.polymerList[ i ], this.getSplineParams())
