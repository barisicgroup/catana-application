...
class StructureBuilder {
  ...
  constructor(readonly structure: Structure) {
    ...
    // CATANA MODIFICATION
    // By default, this constructor was empty.
    // It was modified to allow usage of structure builder
    // on structures which are not empty.
    if (this.structure.modelStore && this.structure.modelStore.count > 0) {
      this.currentModelindex = this.structure.modelStore.count;
    }

    if (this.structure.atomStore) {
      this.ai = this.structure.atomStore.count - 1;
    }

    if (this.structure.residueStore) {
      this.ri = this.structure.residueStore.count - 1;
      this.initRi = this.ri;
    }

    if (this.structure.chainStore) {
      this.ci = this.structure.chainStore.count - 1;
    }

    if (this.structure.modelStore) {
      this.mi = this.structure.modelStore.count - 1;
    }

  }

  get currentChainIndex(): number {
    return this.ci;
  }
  ...
  addAtom(modelindex: number, chainname: string, chainid: string, resname: string, resno: number, hetero: boolean, sstruc?: string | undefined, inscode?: string | undefined) {
    ...
    if (addResidue) {
      ...
      if (this.ri > this.initRi + 1) this.addResidueType(this.ri - 1)
  ...
  finalize() {
    this.previousResname = this.currentResname
    this.previousHetero = this.currentHetero
    if (this.ri > this.initRi) this.addResidueType(this.ri)
  }
}
