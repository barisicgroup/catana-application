...
function getFilt (a: NamedNodeMap, atomname?: string, useAltcode = false) {
  const icode = getNamedItem(a, 'icode').trim()
  const chain = getNamedItem(a, 'chain').trim()
  const altcode = getNamedItem(a, 'altcode')
  let filt = getNamedItem(a, 'resnum')
  if (icode) filt += '^' + icode
  if (chain) filt += ':' + chain
  if (atomname) filt += '.' + atomname
  if (useAltcode && altcode.trim()) filt += '%' + altcode
  filt += '/' + (parseInt(getNamedItem(a, 'model')) - 1)
  return filt
}

function getResFilt (a: NamedNodeMap) {
  const chain = getNamedItem(a, 'chain').trim()
  const rescode = getNamedItem(a, 'rescode')
  const resnum = getNamedItem(a, 'resnum')
  let filt = `[${rescode}]${resnum}`
  if (chain) filt += `:${chain}`
  return filt
}
...
function getAtomFilt (ap: AtomProxy) {
...
class Validation {
  ...
  fromXml (xml: XMLDocument) {
    ...
    if (entries.length === 1) {
      ...
      if (chemicalShiftLists.length === 1) {
        ...
        for (let j = 0, jl = randomCoilIndices.length; j < jl; ++j) {
          ...
          const filt = getResFilt(rcia)
          rciDict[ filt ] = parseFloat(getNamedItem(rcia, 'value'))
        }
      }
    }
    ...
    for (let i = 0, il = groups.length; i < il; ++i) {
      ...
      const filt = getFilt(ga)
      if (ga.getNamedItem('rsrz') !== null) {
        rsrzDict[ filt ] = parseFloat(getNamedItem(ga, 'rsrz'))
      }
      if (ga.getNamedItem('rscc') !== null) {
        rsccDict[ filt ] = parseFloat(getNamedItem(ga, 'rscc'))
      }
      const filtAttr = xml.createAttribute('sele')
      filtAttr.value = filt
      ga.setNamedItem(filtAttr)
      ...
      for (let j = 0, jl = clashes.length; j < jl; ++j) {
        ...
        if (guessElement(atom) !== 'H') {
          ...
          const atomFilt = getFilt(ga, atom, true)
          atomDict[ atomFilt ] = true
    ...
    this.clashFilt = clashList.length ? clashList.join(' OR ') : 'NONE'
  ...
  getClashData (params: { color: number|string|Color, structure: Structure }) {
    ...
    s.eachAtom(function (ap) {
      const filt = getAtomFilt(ap)
      if (atomDict[ filt ] === true) {
        atomDict[ filt ] = ap.index
      }
    })
    ...
    clashArray.forEach(function (c, idx) {
      ap1.index = atomDict[ c.filt1 ] as number  // TODO
      ap2.index = atomDict[ c.filt2 ] as number  // TODO
