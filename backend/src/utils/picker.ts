...
import { Shape, Structure, Volume } from '../catana';
...
class Picker {
  ...
  /**
   * @returns identification of picking object
   */
  get type () { return '' }

  /**
   * @returns data associate with picking object
   */
  get data () { return {} }
...
class ClashPicker extends Picker {
  ...
  _getAtomProxyFromFilt (filt: string) {
    const filter = new Filter(filt)
    const idx = this.structure.getAtomIndices(filter)![ 0 ]
    return this.structure.getAtomProxy(idx)
  }

  _getPosition (pid: number) {
    const clash = this.getObject(pid).clash
    const ap1 = this._getAtomProxyFromFilt(clash.filt1)
    const ap2 = this._getAtomProxyFromFilt(clash.filt2)
