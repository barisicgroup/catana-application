...
class ResnameColormaker extends Colormaker {
  // Catana additions and modifications
  atomColor (a: AtomProxy) {
    return ResnameColormaker.getColor(a.resname);
  }
  monomerColor(m: CgMonomerProxy): number {
    return ResnameColormaker.getColor(m.residueName);
  }
  private static getColor(resname: string): number {
    return ResidueColors[resname] || DefaultResidueColor;
  }
}

ColormakerRegistry.add('resname', ResnameColormaker as any, Object.values(ResidueColors))
