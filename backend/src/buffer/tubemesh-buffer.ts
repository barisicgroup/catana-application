...
export const TubeMeshBufferDefaultParameters = Object.assign({
  ...
  capped: false,
  aspectRatio: 1.0,
  arrowEnded: false, // Catana extension
  arrowEndedLength: 4  // Catana extension
  ...
  setAttributes(data: Partial<TubeMeshBufferData> = {}) {
    ...
    for (let i = 0; i < n; ++i) {
      ...
      if (position && tangent && normal && binormal && size) {
        ...
        let targetRadius = size[i];

        if (this.parameters.arrowEnded && i > n - this.parameters.arrowEndedLength) {
          targetRadius = MathUtils.lerp(0.01, targetRadius,
            (n - 1 - i) / this.parameters.arrowEndedLength);
        }
