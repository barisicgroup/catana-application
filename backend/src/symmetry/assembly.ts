...
function filterFromChains (chainList: string[]) {
  let filt = ''
  if (chainList.length > 0) {
    filt = ':' + uniqueArray(chainList).join(' OR :')
  }
...
class Assembly {
  ...
  getFilter () {
    ...
    return filterFromChains(chainList)
  }
}
...
export class AssemblyPart {
  ...
  getBoundingBox (structure: Structure) {
    ...
    const structureBox = structure.getBoundingBox(filter)
  ...
  getFilter () {
    return filterFromChains(this.chainList)
  }

  getView (structure: Structure): Structure | StructureView {
    const filter = this.getFilter()
    if (filter) {
      return structure.getView(filter)
    } else {
      return structure
    }
  }
