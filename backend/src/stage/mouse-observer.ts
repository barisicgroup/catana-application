...
export interface MouseSignals {
  ...
  dragged: Signal  // on drag: deltaX, deltaY, x, y
...
class MouseObserver {
  ...
  getNdcPosition(): Vector2 {
    const ssPos: Vector2 = this.canvasPosition.clone();
  ...
  getWorldPosition(): Vector3 {
    ...
    const worldPos: Vector3 = new Vector3(ndcPos.x, ndcPos.y, 1).unproject(this.stage.viewer.camera);
    ...
    worldPos
      ...
      .sub(this.stage.viewerControls.position);
  ...
  _onMousemove(event: MouseEvent) {
    ...
    if (this.pressed) {
      this.signals.dragged.dispatch(dx, dy, this.canvasPosition.x, this.canvasPosition.y); // Catana modification
    }
  }
  ...
  _onMousedown(event: MouseEvent) {
    ...
    // Catana modification: Prevent mousedown from being fired twice within one drag
    if (this.pressed) {
      return;
    }
  ...
  _onMouseup(event: MouseEvent) {
    ...
    // Catana modification: Prevent mouseup from being fired twice within one drag
    if (!this.pressed) {
      return;
    }
  ...
  _onTouchmove(event: TouchEvent) {
    ...
    switch (event.touches.length) {
      case 1: {
        ...
        this.signals.moved.dispatch(dx, dy)
        if (this.pressed) {
          this.signals.dragged.dispatch(dx, dy, this.position.x, this.position.y); // Catana modification
        }
        break
      }
