...
class MouseActions {
   ...
   * Catana stuff
   */
  static clickPick_left(stage: Stage, pickingProxy: PickingProxy, x: number, y: number) {
    if (stage.viewer.catanaRendering.clickLeft(x, y, stage)) return;
    if (stage.catanaActions.click_left(stage, pickingProxy)) return;
    //MouseActions.movePick(stage, pickingProxy); // TODO select component on click
  }
  static hoverPick(stage: Stage, pickingProxy: PickingProxy, x: number, y: number) {
    if (stage.viewer.catanaRendering.hover(x, y, stage)) return;
    if (stage.catanaActions.hover(stage, pickingProxy)) return;
    MouseActions.tooltipPick(stage, pickingProxy);
  }
  static downPick_left(stage: Stage, pickingProxy: PickingProxy, x: number, y: number) {
    if (stage.viewer.catanaRendering.downLeft(x, y, stage)) return;
    stage.catanaActions.down_left(stage, pickingProxy);
  }
  static upPick_left(stage: Stage, pickingProxy: PickingProxy, x: number, y: number) {
    if (stage.viewer.catanaRendering.upLeft(x, y, stage)) return;
    stage.catanaActions.up_left(stage, pickingProxy);
  }
  static dragPick_left(stage: Stage, pickingProxy: PickingProxy, x: number, y: number, dx: number, dy: number) {
    if (stage.viewer.catanaRendering.dragLeft(x, y, stage)) return;
    if (stage.catanaActions.drag_left(stage, pickingProxy)) return;

    if (stage.mouseObserver.altKey) return;
    if (stage.mouseObserver.shiftKey && stage.mouseObserver.ctrlKey) {
      MouseActions.rotateComponentDrag(stage, dx, dy);
    } else if (stage.mouseObserver.shiftKey) {
      MouseActions.zoomDrag(stage, dx, dy);
    } else if (stage.mouseObserver.ctrlKey) {
...
export const MouseActionPresets = {
  default: [
    ...
    //[ 'drag-left', MouseActions.rotateDrag ], // Now dragPick-left
    [ 'drag-right', MouseActions.panDrag ],
    //[ 'drag-ctrl-left', MouseActions.panDrag ], // Now dragPick-ctrl-left
    [ 'drag-ctrl-right', MouseActions.zRotateDrag ],
    //[ 'drag-shift-left', MouseActions.zoomDrag ], // Now dragPick-shift-left
    [ 'drag-middle', MouseActions.zoomFocusDrag ],

    [ 'drag-ctrl-shift-right', MouseActions.panComponentDrag ],
    //[ 'drag-ctrl-shift-left', MouseActions.rotateComponentDrag ], // Now dragPick-ctrl-shift-left
    ...
    [ 'hoverPick', MouseActions.hoverPick ], //[ 'hoverPick', MouseActions.tooltipPick ],
      // Catana additions
    [ 'hoverPick-shift', MouseActions.hoverPick ],
    [ "downPick-left", MouseActions.downPick_left],
    ...
    [ "dragPick-left", MouseActions.dragPick_left ],
    [ "dragPick-shift-left", MouseActions.dragPick_left ],
    //[ "dragPick-ctrl-left", MouseActions.dragPick_left ],
    //[ "dragPick-ctrl-shift-left", MouseActions.dragPick_left ]
