...
// Catana modification
type CatanaMouseActionType = "downPick" | "upPick" | "dragPick" ;
export type MouseActionType = ''|'scroll'|'drag'|'click'|'doubleClick'|'hover'|'clickPick'|'hoverPick' | CatanaMouseActionType;
...
function triggerFromString (str: string) {
  ...
  // Catana addition
  if (tokens.includes("downPick")) type = "downPick";
  if (tokens.includes("upPick")) type = "upPick";
  if (tokens.includes("dragPick")) type = "dragPick";
...
class MouseControls {
  ...
  // Catana addition: Prevent drag-shift, drag-ctrl, drag-alt, etc... from being fired while dragging
  draggingKey?: number;
  ...
  run (type: MouseActionType, ...args: any[]) {
    ...
    let key = this.mouse.key || 0; // Catana modification: Declare 'key' with 'let' (not 'const')
    const button = this.mouse.buttons || 0

    // Catana addition
    //console.log("type=" + type + ", key=" + key + ", button=" + button);
    if (type === "upPick") {
      if (this.draggingKey !== undefined) key = this.draggingKey;
      this.draggingKey = undefined;
    } else if (type === "downPick") {
      this.draggingKey = key;
    } else if (this.draggingKey !== undefined && this.draggingKey !== key) {
      // Ihere is a draggingKey (i.e., some dragging is taking place)
      // and the draggingKey is different than the key currently being pressed
      // => Keep the draggingKey from the downPick event
